...
| `event.issues`           | performs an analysis to detect issues related to __events__ in blockchain software for `hyperledger-fabric` framework                        |
...
### Event Issues Analysis

```bash
./build/distributions/go-lisa-0.1/bin/go-lisa -i code.go -o output -f hyperledger-fabric -a event-issues
```  
