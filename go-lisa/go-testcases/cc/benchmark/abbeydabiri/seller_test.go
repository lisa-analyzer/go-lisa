...
func TestSellerWR(t *testing.T) {
	fmt.Println("Entering TestSellerLogic")
	...
	writeResp := stub.MockInvoke(uid,
		[][]byte{[]byte(utils.SellerW),
			[]byte("1"),
			[]byte("1"),
			[]byte("Test Seller Type"),
			[]byte("Test Details"),
			[]byte("Test Reg Date"),
		})
	...
	var ccResp struct {
		...
		Seller  string        `json:"message"`
		Payload models.Seller `json:"payload"`
	}
	if err := json.Unmarshal(readResp.GetPayload(), &ccResp); err != nil {
		panic(err)
	}

	assert.Equal(testID, ccResp.Payload.SellerID, "Retrieved Seller ID mismatch")
