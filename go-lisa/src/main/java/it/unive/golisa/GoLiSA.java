...
public class GoLiSA {
								...
								if(raw.getLocation().equals(call.getLocation())
									...
									for( Entry<Pair<String, CallType>, Set<Pair<String, Integer>>> sinks : sinksPhase.entrySet())
										if(call.getCallType().equals(sinks.getKey().getRight())) {
											for(Pair<String, Integer> sign : sinks.getValue()) {
											if(call.getTargetName().equals(sign.getLeft()) 
													&& call.getParameters().length > sign.getValue())
												for (CFGWithAnalysisResults<
														GoAbstractState<ValueEnvironment<TaintDomain>, TypeEnvironment<InferredTypes>>,
														GoPointBasedHeap, ValueEnvironment<TaintDomain>,
														TypeEnvironment<InferredTypes>> result : tool.getResultOf(call.getCFG()))
																if (result.getAnalysisStateAfter(call.getParameters()[sign.getRight()]).getState().getValueState()
																.getValueOnStack().isTainted() || result.getAnalysisStateAfter(call.getParameters()[sign.getRight()]).getState().getValueState()
																.getValueOnStack().maybeTainted())
																	resultsParam[sign.getRight()] = true;		
											}
										}
	...
	private static Collection<Pair<Set<Call>, Set<Call>>> extractPossiblePrivateCollectionConflicts(Map<Call, Tarsis>  collectionsReadPrivateState,
			...
			for(Call key2 :collectionsReadPrivateState.keySet()) {
				...
				if(!readerSetsMapping.values().stream().anyMatch(s -> s.contains(key1)) 
					...
					&& !stringValue1.isTop() && !stringValue2.isTop() 
					 && stringValue1.getAutomatonString().mayBeEqualTo(stringValue2.getAutomatonString())) {
					similarCollections.add(key2);
				}
			}
			
			if(stringValue1.isTop()) {
				readerSetsMapping.putIfAbsent(Tarsis.TOP, new HashSet<Call>());
				readerSetsMapping.get(Tarsis.TOP).add(key1);
			} else 			
		...
		for(Call key1 : collectionsWritePrivateState.keySet()) {
			...
			if(stringValue1.isTop()) {
				writerSetsMapping.putIfAbsent(Tarsis.TOP, new HashSet<Call>());
				writerSetsMapping.get(Tarsis.TOP).add(key1);
			} else
		...
		for(Tarsis stringValueReaders : readerSetsMapping.keySet()) {
			...
			if(stringValueReaders.equals(Tarsis.TOP)) {
				for(Set<Call> ww : writerSetsMapping.values())
					writers.addAll(ww);
			} else {
				for(Tarsis stringValueWriters : writerSetsMapping.keySet()) {
					if(stringValueWriters.equals(Tarsis.TOP) 
							|| !stringValueReaders.getAutomatonString().mayBeEqualTo(stringValueWriters.getAutomatonString()))
