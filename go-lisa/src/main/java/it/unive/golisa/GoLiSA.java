...
public class GoLiSA {
	...
	public static void main(String[] args) throws AnalysisSetupException {
		...
		Option contractInfo = new Option("ci", "contractinfo", true,
				"the deployment name and channel of the contract to analyze (Hyperledger Fabric only)");
		...
		Option crosscontract_opt = new Option("xc", "crosscontract", false,
				"set cross-contract analysis (Hyperledger Fabric only)");
		...
		for (FileInfo fInfo : fileInfos) {
			...
			Program program = lisaExecution(fInfo.getInput(), annotationSet, cmd.getOptionValue("framework"), analysis,
					conf);
			...
			if (require2Phase) {
				...
				case "ucci":
					if (ucciChecker.getUCCIs().size() > 0) {
						conf.openCallPolicy = RelaxedInformationFlowOpenCallPolicy.INSTANCE;
						conf.analysis = simpleDomain(new PointBasedHeap(), new UCCITaintDomain(ucciChecker.getUCCIs()),
								defaultTypeDomain());
						conf.semanticChecks.add(new TaintChecker<>(
								"Possible operation with untrusted information coming from an untrusted cross-contract invocation."));
					}
					break;
				case "read-write":
					conf.openCallPolicy = ReturnTopPolicy.INSTANCE;
			...
			if (crossContractAnalysis) {
				...
				case "cchi":
					...
					for (FileInfo fi : fileInfos) {
						...
						cchis2.workdir = conf.workdir + File.separatorChar + "xcontract" + File.separatorChar + "Result"
								+ fileInfos.hashCode();
						...
						if (cchisToCheck != null && !cchisToCheck.isEmpty()) {
							cchis2.semanticChecks
									.add(new CrossChannelInvocationsWriteOpsAndEventEmitChecker<>(cchisToCheck,
											cmd.hasOption(dumpAdditionalAnalysisInfo)));
							lisaExecution(fi.getInput(), annotationSet, cmd.getOptionValue("framework"), "cchi-write",
									cchis2);
						}
					}
	...
	private static Program lisaExecution(String filePath, AnnotationSet[] annotationSet, String framework,
			String analysis,
