	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> forwardSemanticsAux(
		...
		// No type information is provided and just a single type
				...
				if (type instanceof GoSliceType) {
					...
					AnalysisState<A> containerState = interprocedural.getAnalysis().smallStepSemantics(arraySemantics, sliceCreated, this);
					...
					for (SymbolicExpression sliceExp : sliceExps) {
						...
						for (SymbolicExpression lenId : lenState.getExecutionExpressions())
							lenResult = lenResult
									.lub(interprocedural.getAnalysis().assign(lenState, lenId, new Constant(GoIntType.INSTANCE, length, getLocation()), this));
						...
						for (SymbolicExpression lenId : capState.getExecutionExpressions())
							capResult = capResult
									.lub(interprocedural.getAnalysis().assign(capState, lenId, new Constant(GoIntType.INSTANCE, cap, getLocation()), this));
						...
						AnalysisState<A> ptrState = interprocedural.getAnalysis().smallStepSemantics(capResult, ptrAccess, this);
