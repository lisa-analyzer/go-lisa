...
/**
 * A Go greater or equal function call (e1 < e2).
 * The static type of this expression is definitely {@link GoBoolType}.
 * The semantics of Go greater or equal expression follows the Golang specification:
 * {@link https://golang.org/ref/spec#Comparison_operators}
 * 
 * @author <a href="mailto:vincenzo.arceri@unive.it">Vincenzo Arceri</a>
 */
public class GoGreaterOrEqual extends BinaryNativeCall {

	/**
	 * Builds a Go greater or equal expression at a given location in the program.
	 * 
	 ...
	 */
	public GoGreaterOrEqual(CFG cfg, SourceCodeLocation location, Expression exp1, Expression exp2) {
		...
		for (Type leftType : leftExp.getTypes())
			for (Type rightType : rightExp.getTypes()) {
				if (leftType.canBeAssignedTo(rightType) || rightType.canBeAssignedTo(leftType)) {
					...
					AnalysisState<A, H, V> tmp = rightState.smallStepSemantics(new BinaryExpression(Caches.types().mkSingletonSet(GoBoolType.INSTANCE), leftExp, rightExp, BinaryOperator.COMPARISON_GE, getLocation()), this);
