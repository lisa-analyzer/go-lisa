	...
	public static class UnmarshalImpl extends BinaryExpression
		...
		T extends TypeDomain<T>> AnalysisState<A, H, V, T> semanticsForTaintDomain(AnalysisState<A, H, V, T>  state, AnalysisState<A, H, V, T>  rightState,
			...
			if (rightEnv != null || leftEnv != null) {
					...
					if(rightEnv != null)
						for( SymbolicExpression reWriteExpr : rightState.rewrite(rightState.getComputedExpressions(), original)) {
							NonRelationalValueDomain<?> stack = rightEnv.eval((ValueExpression) reWriteExpr, original);
					...
					if(leftEnv != null)
						for( SymbolicExpression reWriteExpr : leftState.rewrite(leftState.getComputedExpressions(), original)) {
