...
public class GoMultiAssignment extends Expression {
	...
	private <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> assignScopedId(InterproceduralAnalysis<A, D> interprocedural, AnalysisState<A> rightState,
		...
		for (int i = 1; i < blocksBeforeDecl.size(); i++) {
			...
			for (int j = blocksBeforeDecl.size() - 1 - i; j >= 0; j--)
				tmp = tmp.pushScope(new ScopeToken(blocksBeforeDecl.get(j).getOpen()), this);
			tmp = interprocedural.getAnalysis().assign(tmp, idToAssign, expr2, this);
		}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> forwardSemantics(
		...
		for (int i = 0; i < ids.length; i++) {
			...
			for (SymbolicExpression retExp : rightState.getExecutionExpressions()) {
				...
				for (SymbolicExpression idExp : idState.getExecutionExpressions()) {
					AnalysisState<A> assign = assignScopedId(interprocedural, rightState, idExp,
				...
				for (SymbolicExpression idExp : idState.getExecutionExpressions()) {
					...
					if (rightExpType instanceof ReferenceType
						...
						if (typeAtPos instanceof ReferenceType) {
							...
							assign = interprocedural.getAnalysis().assign(finalResult, idExp, ref, this);
						} else
							assign = interprocedural.getAnalysis().assign(finalResult, idExp, access, this);
					} else
						assign = interprocedural.getAnalysis().assign(finalResult, idExp, access, this);
