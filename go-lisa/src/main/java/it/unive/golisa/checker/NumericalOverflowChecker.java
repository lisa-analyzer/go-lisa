...
/*		
		else if (node instanceof GoMinus) {
...
*/
		...
		if (leftExpression instanceof VariableRef) {
			...
			for (Type type : id.getRuntimeTypes(leftExpression.getProgram().getTypes()))
			...
			for (AnalyzedCFG<SimpleAbstractState<PointBasedHeap, ValueEnvironment<Interval>, TypeEnvironment<InferredTypes>>, PointBasedHeap, ValueEnvironment<Interval>, TypeEnvironment<InferredTypes>> an : tool.getResultOf(graph)) {
				...
				ValueEnvironment<Interval> interval = (ValueEnvironment<Interval>) analysisAtNode.getState().getValueState();				
				...
				try {
					...
					overflows = interval.satisfies(checkOver, node);
					...
					underflows = interval.satisfies(checkUnder, node);
