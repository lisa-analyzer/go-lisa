					...
					if(multiAssign.getIds().length == 2)
						if(multiAssign.getIds()[1] instanceof VariableRef) {
							...
							if(GoLangUtils.isBlankIdentifier(ref.getVariable()))
								tool.warnOn(node, "Unhandled error of a blockchain " + (ReadWriteHFUtils.isReadCall((Call) expr) ? "read" : "write") + " operation. It is discarded during the assignment.");
							else {
								// TODO: check if the err variable is used in a conditional statement
								boolean found = false;
								for(ControlFlowStructure cfs :graph.getControlFlowStructures()) {
									if(cfs instanceof IfThenElse) {
										if(CFGUtils.existPath(graph, node, cfs.getCondition(), Search.BFS)
												//TODO: add condition to avoid overwrite of err
												) {
											if(isVariableRefUsedInCondition(multiAssign.getIds()[1], cfs.getCondition())) {
												found= true;
												break;
											}
										}
								...
								if(!found)
									tool.warnOn(node, "Unhandled error of a blockchain " + (ReadWriteHFUtils.isReadCall((Call) expr) ? "read" : "write") + " operation. It seems not checked.");	
							}
						}
